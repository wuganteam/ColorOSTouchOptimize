...
- 🎮 **游戏高采样率**：默认 320Hz，毫秒级响应
- 🔋 **日用低采样率**：默认 125Hz，省电续航
...
```
# 全局配置（必需，仅一行）
config 游戏采样率=6, 日用采样率=0, touch_node=182

# 游戏列表（每行一个包名）
com.tencent.tmgp.sgame           # 使用全局游戏采样率
com.miHoYo.hkrpg rate=8          # 该游戏使用独立采样率 8
```

| 参数 | 说明 | 默认值 |
|------|------|--------|
| `游戏采样率`/`game_rate` | 游戏运行时采样率 | 6 → 320Hz |
| `日用采样率`/`default_rate` | 日常采样率 | 0 → 125Hz |
